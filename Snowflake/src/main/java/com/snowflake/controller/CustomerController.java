...
public class CustomerController {
    ...
    public ResponseEntity<String> generateDemoData(@PathVariable int count) {
        logger.info("Generating {} demo customer records", count);
        Instant start = Instant.now();
        customerService.generateDemoData(count);
        Instant end = Instant.now();
        logger.info("Inserted {} records in {} ms", count, Duration.between(start, end).toMillis());
        return ResponseEntity.ok("Inserted " + count + " demo records successfully.");
    }
}